...
onMounted(async () => {
    try {
        ...
        const DataOfNoteEdited = await fetch(`${import.meta.env.VITE_API_URL}/notes/${id.value}`)
        const NoteEdited = await DataOfNoteEdited.json()
        form.note.title = NoteEdited.data.title
        form.note.note = NoteEdited.data.note
        form.note.category = NoteEdited.data.category.category
...
function EditNote(){
    try {
        const data = {
            ...
            userId : parseInt(id.value),
        }
        axios.put(`${import.meta.env.VITE_API_URL}/notes/${id.value}`, data).then(response =>{
...
<template>
    <div class="flex justify-center items-center min-h-screen bg-gray-100 p-4">
      ...
      <form v-else class="w-full max-w-lg bg-white p-6 rounded-lg shadow-md">
        ...
        <button @click.prevent="EditNote" class="button w-[20%] bg-green-500 p-3 mt-4 rounded-md cursor-pointer hover:bg-green-400">Edit</button>
      </form>
    </div>
  </template>